...
def scale_setup_fcn(pmm, mset, sset, ikts):
    ...
    ikts.add_ikmarkertask('C7', True, 50.0)
    ikts.add_ikmarkertask('CLAV', True, 50.0)
    ...
    ikts.add_ikmarkertask_bilateral('LFC', True, 750.0)
    ikts.add_ikmarkertask_bilateral('MFC', True, 750.0)
...
def add_to_study(study):
    subject = study.add_subject(3, 64.00188)

    cond_args = dict()
    cond_args['run500'] = (3, '_newCOP3')
    ...
    # walk2_trial.add_task(osp.TaskGRFGaitLandmarks, threshold=5)

    # Trial to use
    gait_events = dict()
    gait_events['right_strikes'] = [0.177, 1.274, 2.372, 3.468]
    gait_events['left_toeoffs'] = [0.360, 1.457, 2.553]
    gait_events['left_strikes'] = [0.715, 1.817, 2.902]
    gait_events['right_toeoffs'] = [0.892, 1.990, 3.099]
